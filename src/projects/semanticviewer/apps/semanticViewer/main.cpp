...
int main(int ac, char** av) 
{
	...
	// Create the ULR view.
	GaussianView::Ptr	semanticView(new GaussianView(scene, sceneResWidth, sceneResHeight, plyfile.c_str(), &messageRead, sh_degree, white_background, !myArgs.noInterop, device));
	...
	multiViewManager.addIBRSubView("Semantics view", semanticView, usedResolution, ImGuiWindowFlags_ResizeFromAnySide | ImGuiWindowFlags_NoBringToFrontOnFocus);
	multiViewManager.addCameraForView("Semantics view", generalCamera);
	if (myArgs.showRGB) {
		GaussianView::Ptr gaussianView(new GaussianView(scene, sceneResWidth, sceneResHeight, plyfile.c_str(), &messageRead, sh_degree, white_background, !myArgs.noInterop, device, false));
	...
	// save images
	generalCamera->getCameraRecorder().setViewPath(semanticView, myArgs.dataset_path.get());
	if (myArgs.pathFile.get() !=  "" ) 
	{
		generalCamera->getCameraRecorder().loadPath(myArgs.pathFile.get(), usedResolution.x(), usedResolution.y());
		generalCamera->getCameraRecorder().recordOfflinePath(myArgs.outPath, multiViewManager.getIBRSubView("Semantics view"), "");
		if( !myArgs.noExit )
			exit(0);
	}
