...
#include <projects/semanticviewer/renderer/GaussianView.hpp>
...
inline std::unique_ptr<std::istream> read_binary(
...
int loadPly(const char* filename,
	...
	bool sematnic = false)
{
	std::unique_ptr<std::istream> ply_stream_buffer = read_binary(filename);
	tinyply::PlyFile file;
  std::shared_ptr<tinyply::PlyData> vert, feat, opac, scal, rota;
  file.parse_header(*ply_stream_buffer);

	SIBR_LOG << "\t[ply_header] Type: "
            << (file.is_binary_file() ? "binary" : "ascii") << std::endl;
  for (const std::string& c : file.get_comments())
    SIBR_LOG << "\t[ply_header] Comment: " << c << std::endl;
  for (const std::string& c : file.get_info())
    SIBR_LOG << "\t[ply_header] Info: " << c << std::endl;

  for (const tinyply::PlyElement& e : file.get_elements()) {
    SIBR_LOG << "\t[ply_header] element: " << e.name << " (" << e.size << ")"
              << std::endl;
    for (const tinyply::PlyProperty& p : e.properties) {
			std::stringstream ss;
      ss << "\t[ply_header] \tproperty: " << p.name
                << " (type=" << tinyply::PropertyTable[p.propertyType].str
                << ")";
      if (p.isList)
        ss << " (list_type=" << tinyply::PropertyTable[p.listType].str
                  << ")";
      SIBR_LOG << ss.str() << std::endl;
    }
  }
  ...
  try {
    vert = file.request_properties_from_element("vertex", {"x", "y", "z"});
  } catch (const std::exception& e) {
    SIBR_ERR << "tinyply exception: " << e.what() << std::endl;
  }

  try {
		std::vector<std::string> feat_names((D+1)*(D+1)*3);
		std::string prefix = "f_";
		if (sematnic) prefix = "mask_";
		for(int i = 0; i < 3; i++) {
				feat_names[i] = prefix + "dc_" + std::to_string(i);
			}
		for (int i = 0; i < (D+1)*(D+1)*3 - 3; i++) {
			feat_names[i+3] = prefix + "rest_" + std::to_string(i);
		}
		feat = file.request_properties_from_element("vertex", feat_names);
  } catch (const std::exception& e) {
    SIBR_ERR << "tinyply exception: " << e.what() << std::endl;
  }

	try {
		opac = file.request_properties_from_element("vertex", {"opacity"});
	} catch (const std::exception& e) {
		SIBR_ERR << "tinyply exception: " << e.what() << std::endl;
	}

	try {
		scal = file.request_properties_from_element("vertex", {"scale_0", "scale_1", "scale_2"});
	} catch (const std::exception& e) {
		SIBR_ERR << "tinyply exception: " << e.what() << std::endl;
	}

	try {
		rota = file.request_properties_from_element("vertex", {"rot_0", "rot_1", "rot_2", "rot_3"});
	} catch (const std::exception& e) {
		SIBR_ERR << "tinyply exception: " << e.what() << std::endl;
	}
  ...
  if (vert) {
    SIBR_LOG << "\tRead " << vert->count << " total vertices "
              << std::endl;
    try {
      pos.resize(vert->count);
      std::memcpy(pos.data(), vert->buffer.get(),
                  vert->buffer.size_bytes());
    } catch (const std::exception& e) {
      SIBR_ERR << "tinyply exception: " << e.what() << std::endl;
    }
  } else {
    SIBR_ERR << "Error: vertices not found" << std::endl;
    exit(0);
  }

	if (feat) {
    SIBR_LOG << "\tRead " << feat->count << " total vertices "
              << std::endl;
    try {
      shs.resize(feat->count);
      std::memcpy(shs.data(), feat->buffer.get(),
                  feat->buffer.size_bytes());
    } catch (const std::exception& e) {
      SIBR_ERR << "tinyply exception: " << e.what() << std::endl;
    }
  } else {
  ...
  if (opac) {
		SIBR_LOG << "\tRead " << opac->count << " total vertices "
							<< std::endl;
		try {
			opacities.resize(opac->count);
			std::memcpy(opacities.data(), opac->buffer.get(),
									opac->buffer.size_bytes());
		} catch (const std::exception& e) {
			SIBR_ERR << "tinyply exception: " << e.what() << std::endl;
		}
	} else {
	...
	if (scal) {
		SIBR_LOG << "\tRead " << scal->count << " total vertices "
							<< std::endl;
		try {
			scales.resize(scal->count);
			std::memcpy(scales.data(), scal->buffer.get(),
									scal->buffer.size_bytes());
		} catch (const std::exception& e) {
			SIBR_ERR << "tinyply exception: " << e.what() << std::endl;
		}
	} else {
	...
	if (rota) {
		SIBR_LOG << "\tRead " << rota->count << " total vertices "
							<< std::endl;
		try {
			rot.resize(rota->count);
			std::memcpy(rot.data(), rota->buffer.get(),
									rota->buffer.size_bytes());
		} catch (const std::exception& e) {
			SIBR_ERR << "tinyply exception: " << e.what() << std::endl;
		}
	} else {
	...
	{
		maxx = maxx.cwiseMax(pos[i]);
		minn = minn.cwiseMin(pos[i]);
		// Activate alpha
		opacities[i] = sigmoid(opacities[i]);
		// Exponentiate scale
		for(int j = 0; j < 3; j++) {
			scales[i].scale[j] = exp(scales[i].scale[j]);
		}
		// Normalize quaternion
		float length2 = 0;
		for (int j = 0; j < 4; j++)
			length2 += rot[i].rot[j] * rot[i].rot[j];
		float length = sqrt(length2);
		for (int j = 0; j < 4; j++)
			rot[i].rot[j] = rot[i].rot[j] / length;
	}
...
sibr::GaussianView::GaussianView(const sibr::BasicIBRScene::Ptr & ibrScene, uint render_w, uint render_h, const char* file, bool* messageRead, int sh_degree, bool white_bg, bool useInterop, int device, bool semantic) :
	_scene(ibrScene),
	...
	sibr::ViewBase(render_w, render_h)
{
	...
	if (sh_degree == 1)
	{
		count = loadPly<1>(file, pos, shs, opacity, scale, rot, _scenemin, _scenemax, _semantic);
	}
	else if (sh_degree == 2)
	{
		count = loadPly<2>(file, pos, shs, opacity, scale, rot, _scenemin, _scenemax, _semantic);
	}
	else if (sh_degree == 3)
	{
		count = loadPly<3>(file, pos, shs, opacity, scale, rot, _scenemin, _scenemax, _semantic);
	}
...
void sibr::GaussianView::onGUI()
{
	...
	std::string guiName = "3D Gaussians";
	if(_semantic) guiName += " Semantic";
